...
export async function getPreviewImages(
  ...
  const imageDocRefs: FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>[] = images
    .map(url => {
      const id = sha256(url);
    ...
    .filter(
      (
        item
      ): item is FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData> =>
        !!item
    );

  if (!imageDocRefs.length || !db.db) {
    return {};
  }
