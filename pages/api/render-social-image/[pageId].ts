...
import { mapNotionImageUrl } from "@utils/map-image-url";
...
export default async (
  ...
  const image = await createSocialImage({
    ...
    logo: mapNotionImageUrl(defaultPageIcon || "", block) || undefined,
    title:
      (isRootPage
        ? socialImageTitle
        : getBlockTitle(block, recordMap) || socialImageTitle) || "",
    subtitle: isRootPage ? socialImageSubtitle || undefined : undefined,
...
async function createSocialImage(params: SocialImageConfig): Promise<any> {
  let browser;
